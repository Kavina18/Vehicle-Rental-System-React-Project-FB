...
export function Login() {
    ...
    async function handleSubmit(event, role) {
        ...
        if (validateForm()) {
            try {
                const response = await axios.post('https://vehicle-rental-system-react-project-fb.onrender.com/api/login', { usermail, password, role });

                if (response.status === 200) {
                    alert(response.data.success);

                    if (role === 'vendor') {
                        navigate('/vendor', { state: { usermail } });
                    } else if (role === 'admin') {
                        navigate("/admin");
        ...
        <div className='login'>
            ...
            <form className='login_form'>
                ...
                <div className='container'>
                    <input type="text" id="usermail" placeholder="Usermail" value={usermail} onChange={(e) => setUsermail(e.target.value)} />
